...
<style>
    ...
    /* Force all text in expander to be white */
    .streamlit-expanderContent * {
        color: #ffffff !important;
    }
...
3. Требуй обязательное логирование всех транзакций"""),
    ...
    **Настройка в GitLab:**
    ...
    # Automatic feedback info
    st.markdown('<div class="section-header">⚙️ Автоматическое обучение</div>', unsafe_allow_html=True)
    
    st.markdown("""
    **🔄 Reaction Poller работает автоматически!**
    
    **Как это работает:**
    1. Backend каждые 60 секунд проверяет reactions на AI комментариях
    2. Сеньор ставит 👍 или 👎 на комментарий AI в GitLab MR
    3. В течение минуты backend обнаружит reaction
    4. Автоматически создается feedback
    5. Negative feedback (👎) → создается learning pattern
    6. Learning pattern добавляется в промпт при следующих анализах
    
    **✅ Ничего не нужно настраивать - уже работает!**
    
    **💡 Как оставить feedback:**
    - Найди комментарий AI (с маркером 🤖)
    - Нажми на 😊 (emoji picker) справа от комментария
    - Выбери 👍 если AI прав или 👎 если ошибся
    - В течение минуты feedback появится в статистике выше
    """)
